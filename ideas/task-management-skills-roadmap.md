...
| Task Author                      | Capability discovery, deterministic drafting, assessment provenance, digest-gated create/revise/normalize, verified two-phase fallback migration, scoring, optional collapsed public score audits, T01-T17, R01-R02, exact verification, and full-native live Task/Bug/Feature creation evidence exist. The canonical Task, Bug, and Feature bodies and required completion pull-request contract are accepted. | Exercise draft, create, revise, normalize, and fallback migration against representative disposable issues; use the results to decide whether any material contract change is still warranted. |
...
| GitHub Issue Form Author         | Task, Bug, and Feature rendering, shared low-friction organization/personal evidence questions, lossless intake handoff, repository inspection, deterministic managed diffs, digest-authorized writes, exact verification, and live alignment evidence exist. All three human intake forms and their lossless mappings are accepted and aligned in the disposable repository.                                   | Later consider organization-default distribution.                                                                                                                                              |
| Task Completion Check            | Read-only completion assessment covers structured acceptance criteria, required linked pull requests, reviews, checks, bounded failure evidence, and the missing-pull-request pending state.                                                                                                                                                                                                                    | Run one representative convergence projection after the Task Author lifecycle exercises; rework only if shared completion evidence materially changes.                                         |
| Phase 1 and phase 2 convergence  | Core implementation, schema live proofs, and the distinct but convergent human-facing and canonical Task, Bug, and Feature templates are complete. The accepted issue forms are aligned live; the final Task Author lifecycle and cross-surface fixture pass remain.                                                                                                                                            | Finish Task Author's representative lifecycle exercises and run one complete fixture pass before declaring the convergence gate passed.                                                        |
...
The installed Task Author also created and exactly verified [Task #99](https://github.com/tanaabased/agent-system-test/issues/99) after adopting the evidence-aware Urgency None policy. The original task recorded Work size `21`, Complexity High, Impact Medium, and Task score `30`; it left Priority and dates unset and published neither fallback metadata nor a scoring-audit comment. After a human changed Impact to Very high, a revise-mode rescore correctly computed Task score `50` but exposed that GitHub treats `issue_field_values` as a complete replacement set: the first update cleared the unchanged Work size, Complexity, and Impact fields. Task Author now builds a complete replacement set for revise and fallback-migration writes, and the repaired live issue exactly retains Work size `21`, Complexity High, Impact Very high, and Task score `50`. Together these proofs show that reviewed absence of an urgency signal can complete `task-score/v1` without inventing a sequencing override and that rescoring preserves every observed issue field.
...
On 2026-08-14, the installed Issue Form Author aligned the accepted low-friction Task, Bug, and Feature forms in `tanaabased/agent-system-test` through an exact digest-authorized three-update plan. The chooser configuration was already aligned, no unmanaged files or deletions were involved, and a fresh plan verified all four managed files with zero operations. [Task #105](https://github.com/tanaabased/agent-system-test/issues/105) remains evidence of the superseded form projection; a new UI submission is required to test the accepted form. Project Optimizer now inventories project-management surfaces that applicable canon explicitly requires even when their files are absent, while leaving form and remote-schema comparison and mutation with their owning skills.
...
Completed: the distinct human-facing and canonical normalized templates for Task, Bug, and Feature and their lossless evidence mappings are accepted, and the disposable-repository forms are exactly aligned. The remaining phase-1 proof is a representative live Task Author lifecycle exercise covering draft, create, revise, normalize, and fallback migration plus the final cross-surface fixture pass.
...
The Task, Bug, and Feature two-layer templates are accepted. Recheck all three task kinds together at the gate, using representative lifecycle and cross-kind fixture evidence to identify any final material inconsistency rather than reopening templates by default. Passing the gate records the accepted two-layer template contract as converged for routine use; later changes require an explicit contract revision rather than incidental scaffolding work.
...
4. **Completed:** accept the canonical Task, Bug, and Feature bodies, their low-friction human forms, lossless evidence mappings, and required completion pull-request gate.
5. **Next:** exercise Task Author's draft, create, revise, normalize, and fallback-migration surfaces against representative disposable issues, then record whether the evidence warrants any material contract change.
6. Run one complete fixture pass across the three authoring skills plus Task Completion Check, confirm the accepted disposable-repository forms remain aligned, and declare the convergence gate passed only if the pass produces no material contract, schema, form, or scoring changes.
...
The ownership, schema, mutation architecture, and two-layer Task, Bug, and Feature templates are settled. The main remaining question is empirical: representative Task Author lifecycle exercises and one final cross-surface fixture pass must establish that provenance, scoring order, Complexity classifications, native/fallback behavior, and normalization match human judgment without requiring a material contract change.
