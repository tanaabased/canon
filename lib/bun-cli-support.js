...
/**
 * Parses shared codexsync flags while preserving the first positional value as
 * the command and returning any remaining positionals for command-level errors.
 *
 ...
 * @throws {Error} When an option is unknown or a required option value is missing.
 */
...
/**
 * Renders the canonical codexsync help surface using the resolved defaults that
 * the current invocation will actually use.
 *
 * @param {object} context Resolved CLI metadata and default paths.
 * @param {string} context.cachePath Managed plugin cache path shown as the option default.
 * @param {string[]} context.commands Supported command names.
 * @param {string} context.cliName Display name for the executable.
 * @param {string} context.repoRoot Source repo root shown as the option default.
