...
async function validateSkills({ failures, repoRoot }) {
  ...
  if (!existsSync(validateSkillScript)) {
    appendFailure(
      failures,
      `missing skill validator: ${path.relative(repoRoot, validateSkillScript)}`,
    );
    return;
  }
...
async function validateWorkflowPackageScripts({ failures, repoRoot }) {
  ...
  for (const workflowFile of workflowFiles) {
    ...
    for (const scriptName of extractPackageScriptNames(content)) {
      if (!scripts.has(scriptName)) {
        appendFailure(
          failures,
          `workflow ${path.relative(repoRoot, workflowFile)} calls missing package script: ${scriptName}`,
        );
      }
    }
  }
