...
- `[manual]` When the skill has an Optimization facet, link its concrete adoption checks to its Preferred Tools. Compare observed usage, compatibility, behavioral differences, migration effort, and maintenance benefit. Report already adopted, worthwhile migration, retain existing implementation, or insufficient evidence only for relevant candidates; omit unrelated tools. A preference alone does not establish drift.
- `[manual]` Use documented compatibility requirements and repository evidence for ordinary optimization; do not turn each pass into an upstream catalog scan. Use the manually invoked [tool recommendation refresh prompt](../prompts/refresh-preferred-tools.md) to review new releases or explicitly selected sources. Keep unreleased, incompatible, or unverified candidates out of adopted guidance.
