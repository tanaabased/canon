...
- Apply the [documentation change gate](./documentation-standards.md#documentation-change-gate) to every task kind before promising prose. Prefer adequate existing or upstream evidence over new tests, fixtures, guides, or CI scaffolding that merely re-prove it; add coverage when the changed local behavior needs it.
