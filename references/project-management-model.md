...
- Task Decomposer owns keep-or-split review and the approved shallow child-task, native hierarchy, dependency, and parent-rollup lifecycle for one oversized task. Project Milestone Planner may hand off an oversized task to that workflow and consume its resulting graph, but decomposition does not select or change milestone membership.
