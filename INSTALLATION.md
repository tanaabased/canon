...
Requires Node and npm supported by [Codex Tools](https://github.com/tanaabased/codex-tools/blob/v1.0.2/CLI.md#invocation). Codex Tools 1.0.2 or newer provisions its own verified Codex CLI; you do not need a separate CLI on `PATH` for this installation path.
...
When switching an existing installation from archives, npm, or another checkout, retain its marketplace identity and replace its source rather than installing a second copy. See [Codex Tools installation options](https://github.com/tanaabased/codex-tools/blob/v1.0.2/CLI.md).
