...
- Keep npm, ClawHub, plugin-archive, and repository publication as independent peer jobs. Share release version and preparation intent; repository publication is a destination, not a finalizer.
- In every publication job, set checkout `ref: ${{ github.sha }}` and `fetch-depth: 0`. The event SHA keeps all jobs on the original release commit even if `publish-repo` moves the tag before another job checks out; full history supplies branches and tags for preparation and synchronization. Fetch depth alone does not prevent the race.
- Use `publish-<destination>` job IDs and matching `Publish <destination>` display names: `publish-npm` / `Publish npm`, `publish-clawhub` / `Publish ClawHub`, `publish-plugin` / `Publish Codex plugin`, and `publish-repo` / `Publish repo`. Use the same destination wording for live publishing steps; avoid sync/finalizer labels for repository publication. Preserve check identities that are bound to repository rules until those rules can be updated together.
...
- In PRs, validate real prepared artifacts with native dry runs; leave publication credentials and external writes to real release workflows. Apply [verification boundaries](../../references/verification-boundaries.md) to keep completion checks inside the owning actions and avoid duplicate post-publication probes. Do not recreate a disposable registry publication harness; dry runs do not prove live publication.
