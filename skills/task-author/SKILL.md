...
12. For an explicitly requested fallback migration, preview two phases and confirm the plan is limited to that mode. Write native values, re-read them, and only then remove the verified fallback keys. Read retired v1 keys only to omit them from the newly rendered v2 capsule; never write them to a native field. Keep conflicts or unavailable supported representations in the capsule and stop for fresh direction on any material surprise.
