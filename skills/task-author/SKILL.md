...
- Treat an explicit create, revise, normalize, or fallback-migration imperative for one exact task as authorization for that bounded mode; keep planning, drafting, previewing, questions, and exploratory discussion read-only.
...
- Assess Work size, Complexity, and Impact from supported evidence before invoking the deterministic helper.
- Preview native issue type and issue-field values, fallback metadata, existing labels, relationships, assessment evidence, and comments.
...
- Apply [the task management contract](../../references/task-management-contract.md) and its [fixture corpus](../../references/task-management-fixtures.md). Do not independently redefine task shapes, metadata, or labels.
...
- Every accepted metadata value requires an `assessment` entry with `source` equal to `agent`, `human`, `policy`, or `existing`. Agent estimates require a concise evidence-based `rationale`.
...
- For agent-owned command execution, prefer `--input -` and send the JSON request through the execution session's standard input. Start the Bun command directly; do not use `printf`, `echo`, a here-document, or a shell pipeline to feed the request because that can embed publication data in a command approval rule. Request elevated execution only for the stable Task Author command prefix, never for request content. If the harness cannot provide standard input, use a repository-local ignored scratch path under the active workspace, verify the intended path with `git check-ignore` before writing, and do not add a tracked placeholder merely to retain the directory.
...
- Show the resolved target, normalized title and body, expected delivery evidence, effective metadata values, assessment provenance and rationale, native plan, fallback-only capsule, unresolved metadata, desired/applicable/missing labels, relationships, planned comments, capability evidence, and warnings.
...
1. Resolve and display one exact target. Stop if it remains ambiguous. Draft only the supported outcome and necessary obligations under the shared scope rules.
2. Before rendering the publication preview, discover and read the authoring agent's applicable voice skill from the available skills; do not select a voice from the assignee or impose a Canon-wide persona. An explicitly requested voice or house style wins; use plain prose when no voice skill is available. Preserve facts, uncertainty, quotations, technical identifiers, acceptance conditions, and permission boundaries; keep sensitive and machine-facing content plain. Trim repetition after styling, then freeze the text for preview and digest approval. A later wording change requires a fresh digest.
3. Run the bundled draft or create command with JSON on standard input. If standard input is unavailable, verify one repository-local ignored scratch path with `git check-ignore` before writing the request there. Both commands check `gh`, read repository ownership, and inspect only the applicable issue-type, organization-field, and repository-label endpoints before any write.
4. Review missing substantive evidence, task-specific verification, authorization boundaries, and metadata errors. Ask focused questions instead of filling gaps with low values or generic acceptance criteria.
5. Review assessment provenance and rationales. Keep agent estimates evidence-based and human-controlled values out of agent ownership.
6. Review native versus fallback planning. Native values win; the fallback capsule includes only proven-unavailable native concepts.
7. Review canonical label intent against observed repository labels. Keep missing or unverified labels unapplied.
...
12. For revision or normalization, re-read current state first, preserve unmanaged labels and earlier comments, and show the complete semantic and storage diff. If an explicit imperative already authorized that exact bounded mode and the diff contains no material surprise, bind its target and digest and continue to PATCH and verification in the same turn; otherwise stop at the preview.
13. For an explicitly requested fallback migration, preview two phases and confirm the plan is limited to that mode. Accept only the current fallback schema, write native values, re-read them, and only then remove the verified fallback keys. Keep conflicts or unavailable supported representations in the capsule and stop for fresh direction on any material surprise.
...
- [./utils/](./utils/): focused assessment, normalization, rendering, metadata, and labeling units
- [./test/](./test/): flat utility, client-boundary, shared-fixture, and revision coverage
